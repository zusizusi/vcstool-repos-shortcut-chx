...
const createRepoButton = (codeLinesElement, repo, top, left) => {
  const versionLineId = `LC${parseInt(repo.key.slice(2)) + 2}`;
...
const displayRepoButtons = (repositories, codeLinesElement) => {
  ...
  Object.values(repositories).forEach((repo) => {
    ...
    createRepoButton(codeLinesElement, repo, rect.top + window.scrollY, textareaRect.left - 50);
  });
};
